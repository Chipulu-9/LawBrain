...
function FeatureCard({
  ...
  return (
    <div className="group relative bg-white border border-brown-200 rounded-2xl p-6 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden">
...
function StepCard({
  ...
  return (
    <div className="flex gap-5">
      <div className="flex-shrink-0 w-10 h-10 rounded-full bg-brown-700 text-brown-50 flex items-center justify-center font-serif font-bold text-lg shadow-md">
        {step}
      </div>
      <div className="pt-1">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          ...
          <h1 className="font-serif text-5xl sm:text-6xl md:text-7xl font-800 text-brown-950 leading-[1.05] tracking-tight mb-6">
            ...
            <span className="relative inline-block">
              ...
              <svg className="absolute -bottom-2 left-0 w-full" viewBox="0 0 400 12" fill="none" preserveAspectRatio="none">
                <path d="M2 9 Q100 2 200 8 Q300 14 398 5" stroke="#c4722a" strokeWidth="3" strokeLinecap="round" fill="none" opacity="0.6"/>
              </svg>
            </span>
          </h1>

          <p className="text-lg sm:text-xl text-brown-600 max-w-2xl mx-auto leading-relaxed mb-12">
            Ask questions about the Zambian Constitution, Acts and Statutes in plain language.
            Every answer is grounded in official legal documents with cited references you can verify.
          </p>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="relative flex items-center bg-white border-2 border-brown-300 rounded-2xl shadow-lg hover:border-brown-500 focus-within:border-brown-700 focus-within:shadow-xl transition-all duration-200">
              ...
              />
              <button className="flex-shrink-0 m-1.5 px-5 py-2.5 bg-brown-700 hover:bg-brown-800 text-brown-50 text-sm font-semibold rounded-xl transition-all shadow-sm hover:shadow-md flex items-center gap-2">
                Ask LawBrain
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              {exampleQueries.slice(0, 3).map(q => (
                <button
                  key={q}
                  onClick={() => setQuery(q)}
                  className="px-3 py-1.5 text-xs font-medium text-brown-600 bg-brown-100 border border-brown-200 rounded-full hover:bg-brown-200 hover:text-brown-800 transition-colors truncate max-w-[220px]"
                >
                  "{q.slice(0, 38)}…"
                </button>
              ))}
              ...
              <div className="bg-brown-50 border border-brown-200 rounded-2xl shadow-xl p-6 space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="bg-white border border-brown-200 rounded-2xl rounded-tl-sm px-4 py-3 text-sm text-brown-800 leading-relaxed shadow-sm max-w-[85%]">
                    <p className="mb-2">
                      Under <strong>Article 18 of the Constitution of Zambia</strong>, every person charged with a criminal offence has the right to:
                    </p>
                    <ul className="space-y-1 text-xs text-brown-700 pl-3">
                      <li className="flex items-start gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-amber-600 mt-0.5 flex-shrink-0" /> Be presumed innocent until proven guilty</li>
                      <li className="flex items-start gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-amber-600 mt-0.5 flex-shrink-0" /> Be informed of the charge in a language they understand</li>
                      <li className="flex items-start gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-amber-600 mt-0.5 flex-shrink-0" /> Have adequate time to prepare a defence</li>
                    </ul>
                    {/* Source badges */}
                ...
                <div className="flex items-center gap-2 mt-2 pt-3 border-t border-brown-100">
                  <div className="flex-1 bg-brown-100 rounded-xl px-4 py-2.5 text-xs text-brown-400 italic">
                    Ask a follow-up question…
                  </div>
                  <button className="p-2 bg-brown-700 rounded-xl text-brown-100 shadow-sm">
                    <ArrowRight className="w-4 h-4" />
                  </button>
                </div>
              </div>
      ...
      <section id="corpus" className="py-20 md:py-28 bg-brown-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-brown-600 max-w-xl mx-auto mb-12">
            LawBrain's knowledge base is built from official, government-published legal documents — not third-party summaries.
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
            {[
              { title: 'Constitution of Zambia', sections: '128 Articles', type: 'Constitution' },
              { title: 'Laws of Zambia — Volume 1', sections: '1,200+ sections', type: 'Compiled Legislation' },
              ...
              <div key={doc.title} className="flex items-start gap-4 bg-white border border-brown-200 rounded-xl p-4 shadow-sm text-left hover:shadow-md transition-shadow">
                <div className="flex-shrink-0 w-10 h-10 bg-brown-100 rounded-lg flex items-center justify-center border border-brown-200">
                  <Scale className="w-5 h-5 text-brown-700" />
                </div>
                <div>
                  <div className="font-serif font-700 text-brown-900 text-sm leading-tight">{doc.title}</div>
      ...
      <section className="brown-gradient py-20 md:py-24">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button className="inline-flex items-center gap-2 px-8 py-4 bg-brown-50 text-brown-900 font-semibold text-sm rounded-xl shadow-lg hover:bg-white transition-all hover:shadow-xl">
              Ask a Question
              <ChevronRight className="w-4 h-4" />
            </button>
            <button className="inline-flex items-center gap-2 px-8 py-4 bg-transparent text-brown-200 border border-brown-500 font-medium text-sm rounded-xl hover:bg-brown-700 transition-all">
              View Legal Corpus
            </button>
          </div>
        </div>
